...

{
  ...
  "version": 1,
  "securityGroups": [],
  "displayName": "Desduplicação evento de compra",
  "brand": {
    "id": "brand_dummy",
  ...
  "description": "Está tag salva nos Cookies do usuário o id da transação realizada buscando dentro da camada de dados ou diretamente do campo padrão.",
  "containerContexts": [
    "WEB"
  ]
}
...
[
  {
    "type": "TEXT",
    "name": "transaction_id",
    "displayName": "Adicione a propriedade da camada de dados que recebe o id da transação",
...
// Salvar o id da transação no storage
// Bloquear a tag de transação do Google Analytics 4

// Triggar o evento de call para tag purchase
addEventCallback(function(ctid, eventData) {
  // Verificando se o usuário não adicionou a opção de pegar a informação da propriedade da camada de dados, se true ele copia diretamente do valor padrão do google, se false ele pega do campo que o usuáiro selecionou.
  let t_id = !data.transaction_id ? copyFromDataLayer("transaction_id") : copyFromDataLayer(data.transaction_id);

  if(t_id) {
    setCookie("transaction_id", t_id);
    return true;
  }

  logToConsole("Log: Não encontrei id de transação");
  return false;
});
...
Created on 3/27/2025, 8:12:34 PM
