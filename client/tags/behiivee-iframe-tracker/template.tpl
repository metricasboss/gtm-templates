___TERMS_OF_SERVICE___

By creating or modifying this file you agree to Google Tag Manager's Community
Template Gallery Developer Terms of Service available at
https://developers.google.com/tag-manager/gallery-tos (or such other URL as
Google may provide), as modified from time to time.


___INFO___

{
  "type": "TAG",
  "id": "cvt_temp_public_id",
  "version": 1,
  "securityGroups": [],
  "displayName": "Behiivee Embed Tracking",
  "brand": {
    "id": "brand_dummy",
    "displayName": "Métricas Boss",
    "thumbnail": "data:image/jpeg;base64,\u003d\u003d"
  },
  "description": "Tracking embed forms",
  "containerContexts": [
    "WEB"
  ]
}


___TEMPLATE_PARAMETERS___

[]


___SANDBOXED_JS_FOR_WEB_TEMPLATE___

// Importar as APIs necessárias
const createQueue = require('createQueue');
const logToConsole = require('logToConsole');
const queryPermission = require('queryPermission');
const injectScript = require('injectScript');
const encodeUriComponent = require('encodeUriComponent');
const getTimestamp = require('getTimestamp');
// Obter valores dos campos configurados
const originDomain = data.originDomain || 'https://embeds.beehiiv.com';
const shouldLog = data.logEvents;
const scriptUrl = data.scriptUrl;

// Registrar início se debug estiver ativado
if (shouldLog && queryPermission('logging')) {
  logToConsole('Beehiiv Event Listener: Iniciando configuração');
  logToConsole('Beehiiv Event Listener: URL do script: ' + scriptUrl);
}

// Verificar permissão para dataLayer
if (queryPermission('access_globals', 'readwrite', 'dataLayer')) {
  // Criar dataLayer se não existir
  createQueue('dataLayer');
  
  // Adicionar parâmetros ao URL para configurar o script
  const fullUrl = scriptUrl + 
                 '?origin=' + encodeUriComponent(originDomain) + 
                 '&debug=' + (shouldLog ? 'true' : 'false') +
                 '&cachebuster=' + encodeUriComponent(getTimestamp());
  
  // Verificar permissão para injetar o script
  if (queryPermission('inject_script', fullUrl)) {
    // Injetar o script
    injectScript(fullUrl, function() {
      // Sucesso
      if (shouldLog && queryPermission('logging')) {
        logToConsole('Beehiiv Event Listener: Script injetado com sucesso');
      }
      data.gtmOnSuccess();
    }, function() {
      // Falha
      if (queryPermission('logging')) {
        logToConsole('Beehiiv Event Listener: Falha ao injetar script');
      }
      data.gtmOnFailure();
    });
  } else {
    if (queryPermission('logging')) {
      logToConsole('Beehiiv Event Listener: Permissão negada para injetar script do URL ' + fullUrl);
    }
    data.gtmOnFailure();
  }
} else {
  if (queryPermission('logging')) {
    logToConsole('Beehiiv Event Listener: Permissão negada para dataLayer');
  }
  data.gtmOnFailure();
}


___WEB_PERMISSIONS___

[
  {
    "instance": {
      "key": {
        "publicId": "logging",
        "versionId": "1"
      },
      "param": [
        {
          "key": "environments",
          "value": {
            "type": 1,
            "string": "debug"
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "access_globals",
        "versionId": "1"
      },
      "param": [
        {
          "key": "keys",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 3,
                "mapKey": [
                  {
                    "type": 1,
                    "string": "key"
                  },
                  {
                    "type": 1,
                    "string": "read"
                  },
                  {
                    "type": 1,
                    "string": "write"
                  },
                  {
                    "type": 1,
                    "string": "execute"
                  }
                ],
                "mapValue": [
                  {
                    "type": 1,
                    "string": "dataLayer"
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  },
                  {
                    "type": 8,
                    "boolean": true
                  }
                ]
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  },
  {
    "instance": {
      "key": {
        "publicId": "inject_script",
        "versionId": "1"
      },
      "param": [
        {
          "key": "urls",
          "value": {
            "type": 2,
            "listItem": [
              {
                "type": 1,
                "string": "https://gtm-templates.s3.us-east-1.amazonaws.com/behiivee-iframe-tracker.js"
              }
            ]
          }
        }
      ]
    },
    "clientAnnotations": {
      "isEditedByUser": true
    },
    "isRequired": true
  }
]


___TESTS___

scenarios: []


___NOTES___

Created on 5/21/2025, 5:02:45 PM


